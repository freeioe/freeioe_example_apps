...
function app:on_start()
	...
	csv_dev.init(self._sys:app_dir())
	...
	for _, v in ipairs(helper:devices()) do
		...
		local dev_tpl, err = csv_dev.load_tpl(v.tpl, tpl_err_cb)
		if not dev_tpl then
			self._log:error("loading csv tpl failed", err)
		end

		if mode_tpl and dev_tpl then
			--- Map the device with template
			local tpl = csv_dev.map_mode(mode_tpl, dev_tpl, tpl_err_cb)
...
function app:on_output(app_src, sn, output, prop, value, timestamp)
	...
	if tpl_output.func then
		val = tpl_output.func:to_raw(val)
	end
		...
		for _, v in ipairs(dev.inputs) do
			...
			if v.fc == tpl_output.fc and v.addr == tpl_output.addr and v.offset == link_offset then
				...
				if v.dt == 'uint8' or v.dt == 'int8' then
					...
					if v.func then
						link_val = v.func:to_raw(link_val)
					end
