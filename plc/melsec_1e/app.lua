--- 导入需要的模块
local m1c_client = require 'm1c.client'
local m1c_types = require 'm1c.frame.types'
local m1c_helper = require 'm1c.helper'
local data_pack = require 'm1c.data.pack'
local data_unpack = require 'm1c.data.unpack'
...
function app:on_start()
	...
	if ioe.developer_mode() then
		...
		config.devs = {
			{ unit = 1, name = 'DEV.1', sn = 'dev1', tpl = 'example', is_m1c1 = false, pack_opt = 'default' }
		}
		config.tpls = {}
	end
	...
	local proto_type = string.upper(config.proto_ver) == 'V1' and m1c_types.PROTO_TYPE_1 or m1c_types.PROTO_TYPE_4
	...
	if conf.channel_type == 'socket' then
		self._client = m1c_client({ link = 'tcp', tcp = conf.opt, proto_type = proto_type }, log)
	else
		self._client = m1c_client({ link = 'serial', serial = conf.opt, proto_type = proto_type }, log)
	end
...
function app:read_packet(dev, stat, unit, pack, tpl)
	...
	local addr_len = m1c_helper.cmd_addr_len(pack.cmd)
	local addr = m1c_helper.make_addr(pack.name, pack.start, addr_len)
