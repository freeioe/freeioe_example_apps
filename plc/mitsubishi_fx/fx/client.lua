...
local stream = require 'fx.stream'
local fx_req = require 'fx.frame.req'
local fx_ack = require 'fx.frame.ack'
local fx_frame_parser = require 'fx.frame.parser'
...
function client:read(dev_no, cmd, start, count, timeout)
	...
	local req = fx_req:new(self._opt.proto_type, dev_no, self._opt.pc_no, cmd, 0, start, count)
	...
	if not result[1] then
		...
		local ack = fx_ack:new(self._opt.proto_type, dev_no, self._opt.pc_no)
...
function client:frame_process()
	while not self._closing do
		...
		frame = fx_frame_parser(tostring(self._buf), 1)
		if not frame then
			self._buf:pop(1)
			goto next_frame
		end
