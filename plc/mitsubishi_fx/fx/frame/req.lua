...
function req:IS_RAND_W()
	return fx_helper.is_random_write(self._cmd)
end
...
function req:valid_hex(raw, index)
	...
	if not fx_helper.is_random_write(cmd) then
		local dlen = fx_helper.cmd_data_len(cmd) 
		local len = HEAD_LEN + fx_helper.cmd_addr_len(cmd) + TAIL_LEN + dlen
		local elen = len
		...
		local dlen = count * ( fx_helper.cmd_addr_len(cmd) + fx_helper.cmd_data_len(cmd) )
...
function req:from_hex(raw, index)
	...
	if not fx_helper.is_random_write(self._cmd) then
		...
		for i = 1, self._count do
			local addr_len = fx_helper.cmd_addr_len(self._cmd)
			local data_len = fx_helper.cmd_data_len(self._cmd)
...
function req:to_hex()
	...
	if not fx_helper.is_random_write(self._cmd) then
		...
		if fx_helper.is_batch_write(self._cmd) then
