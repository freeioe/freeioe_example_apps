...
function calc:set_input_values(values)
	...
	local ei = string.find(str, '\0', 1, true)
	if ei then
...
function calc:start(ua_client)
	...
	if self._enable_sub then
		...
		self._inputs = inputs
		-- Subscribe nodes
		self._log:debug("Create Subscription for MENGLI Meterial Type")
		local r, err = ua_client:create_subscription(inputs, function(input, data_value)
			...
			self._log:debug('Material Type Sub recv', input.ns, input.i, value, data_value.value:asString())
